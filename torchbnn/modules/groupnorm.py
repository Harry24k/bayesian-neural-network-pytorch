...
class BayesGroupNorm(Module):
    ...
    _version = 2
    __constants__ = ['prior_mu', 'prior_sigma', 'eps', 'weight', 'bias', 'num_groups', 'num_channels', 'affine']

    def __init__(self, prior_mu, prior_sigma, num_groups, num_channels, eps=1e-5, affine=True):
        super(BayesGroupNorm, self).__init__()
        ...
        if self.affine:
            ...
            self.weight_mu = Parameter(torch.Tensor(num_channels))
            self.weight_log_sigma = Parameter(torch.Tensor(num_channels))
            self.register_buffer('weight_eps', None)

            self.bias_mu = Parameter(torch.Tensor(num_channels))
            self.bias_log_sigma = Parameter(torch.Tensor(num_channels))
...
#             self.bias_log_sigma.data.fill_(self.prior_log_sigma)
    ...
    def extra_repr(self):
        return '{prior_mu}, {prior_sigma}, {num_groups}, {num_channels}' \
                'eps={eps}, affine={affine}'.format(**self.__dict__)
